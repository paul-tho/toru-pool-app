...
export default function Standings({ setCurrentView, participants, dailyResults }) {
    ...
    const exportPDF = () => {
        ...
        // Tijdens de Tour alleen Totaal; op het eind uitgesplitst
        const columns = tourDone
            ? ['Positie', 'Naam', 'Top 10 pnt', 'Trui pnt', 'Totaal']
            : ['Positie', 'Naam', 'Totaal'];

        const data = standings.map((p, i) => tourDone
            ? [i + 1, p.name, p.topTenPoints, p.jerseyPoints, p.totalPoints]
